...
multipleUploadForm.addEventListener('submit', function(event){
 ...
 var max_fields = 10;
    ...
    var add_button = $(".add_form_article");

    var x = 1;
    $(add_button).click(function(e) {
        e.preventDefault();
         $.ajax({
                url: "/get-form-new-commad-rest",
                type: "GET",
                dataType: "JSON",
                success: function (response) {
                    if (response.hasData) {
                              if (x < max_fields) {
                                             alert('You '+response.produits);

                                        x++;
                                        $(wrapper).append(wrapper.html(response.produits)); //add input box
                                    } else {
...
$(".confirm-valid").click(function(){
    $.post("/confirmation-commande", $("#idfom").serialize(), function(data) {
        alert(data);
    });
});
function showNotif2(contenu, type = 'success', timer = 5000) {
    var rc = 'warning';
    if (type == 'warning') {
        rc = 'warning';
    }
